...
This plan addresses **ALL** complexity, security, and architecture issues across 7 critical files totaling **869 hours** of estimated effort. Each file will be tackled sequentially with full TDD discipline.

**Files in Scope**:
1. `ui/desktop_app.py` - CC 49 (6 complexity issues)
2. `ui/main.py` - CC 16 (3 complexity issues)
3. `scripts/validate_metadata_extraction.py` - CC 34 (6 complexity issues)
4. `scripts/validate_tab5_implementation.py` - CC 34 (1 complexity issue)
5. `workflows/metadata_extraction/scripts/generate_metadata_universal.py` - CC 18 + eval() (3 issues)
6. `workflows/metadata_extraction/scripts/detect_poor_ocr.py` - 3 complexity issues
7. `workflows/pdf_to_json/scripts/chapter_segmenter.py` - 4 complexity issues

**Total Issues**: 26 complexity violations + 1 security issue + 7 files with 0% test coverage
...
1. ✅ Approve TDD approach (Characterization Tests → Refactor)
2. ✅ Approve Strategy Pattern for desktop_app.py (vs Extract Method only)
3. ✅ Approve Domain-Agnostic scope for generate_metadata_universal.py
4. ⏳ **Start with File #1**: generate_metadata_universal.py

**Ready to begin?** Reply with:
- "Start File 1" → Begin generate_metadata_universal.py refactoring
- "Adjust plan" → Modify approach before starting
- "Questions" → Clarify any conflicts or approaches
