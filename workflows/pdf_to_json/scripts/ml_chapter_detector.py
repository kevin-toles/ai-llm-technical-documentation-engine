...
def _find_topic_boundaries(similarities: List[float], 
    ...
    """Find initial chapter boundaries from similarity scores."""
    boundaries = []
    for i, sim in enumerate(similarities):
        # Check if this is a significant topic shift
        if sim < similarity_threshold:
...
def detect_chapters_ml(pages: List[dict], 
    ...
    boundaries = _find_topic_boundaries(similarities, similarity_threshold)
    
    # Step 4: Filter boundaries - enforce minimum chapter length
    filtered_boundaries = _filter_boundaries_by_length(boundaries, min_chapter_length)
    
    # Step 5: Extract chapter titles and build chapter list
    print(f"📖 Found {len(filtered_boundaries)} chapter boundaries")
    return _build_chapter_list(filtered_boundaries, pages, extractor)
