...
**Discussion Points**:
- Traced complete workflow through 7 tabs
...
**Input**: 
- `makinggames_metadata.json` (from Tab 2)
- `makinggames_taxonomy.json` (from Tab 3)
- All other `*_metadata.json` files listed in taxonomy

**Output**: `makinggames_metadata_enriched.json` (50-60 KB)

**Processing Steps**:

1. **Load Cross-Book Context**:
   ```python
   taxonomy = load_json("makinggames_taxonomy.json")
   all_books = []
   for tier in taxonomy["tiers"].values():
       for book_name in tier["books"]:
           metadata = load_json(f"{book_name}_metadata.json")
...
2. **Build Chapter Corpus**:
   ...
   for book in all_books:
       for chapter in book["chapters"]:
           ...
           text += " ".join(chapter['keywords']) + " "
           text += " ".join(chapter['concepts'])
           corpus.append(text)
...
5. **Extract Related Chapters** (per chapter):
   ```python
   for i, chapter in enumerate(current_book_chapters):
       # Get similarity scores for this chapter
       scores = similarity_matrix[i]
       
       # Find top N related chapters (threshold: 0.7+)
       related = []
       for j, score in enumerate(scores):
           if score > 0.7 and i != j:
               related.append({
                   "book": chapter_index[j]["book"],
                   "chapter": chapter_index[j]["chapter"],
                   "title": chapter_index[j]["title"],
                   "relevance_score": round(score, 2),
                   "method": "cosine_similarity"
               })
       
       # Sort by relevance, take top 5
       related = sorted(related, key=lambda x: x["relevance_score"], reverse=True)[:5]
       chapter["related_chapters"] = related
   ```
...
6. **Re-score Keywords** (YAKE with cross-book context):
   ...
   # Get related chapter texts
   related_texts = [get_chapter_text(r) for r in related_chapters]
   combined_text = current_chapter_text + " ".join(related_texts)
   
   # Extract keywords with YAKE
   kw_extractor = yake.KeywordExtractor(top=10)
   keywords_enriched = kw_extractor.extract_keywords(combined_text)
   
   chapter["keywords_enriched"] = [
       {"term": kw, "score": score, "source": "cross_book_yake"}
       for kw, score in keywords_enriched
   ]
   ```
...
7. **Enhance Concepts** (Summa with cross-book context):
   ...
   # Extract concepts from related chapters
   concepts = summa_keywords.keywords(
       combined_text, 
       ratio=0.2,
       scores=True
   )
   
   chapter["concepts_enriched"] = [
       {"concept": concept, "relevance": score, "books": count_books(concept)}
  ...
  "chapters": [
    {
      ...
      "keywords_enriched": [
        {"term": "pygame", "score": 0.92, "source": "cross_book_yake"},
        {"term": "installation", "score": 0.88, "source": "cross_book_yake"}
      ],
      "concepts_enriched": [
        {"concept": "game development", "relevance": 0.95, "books": 3},
        {"concept": "module management", "relevance": 0.82, "books": 5}
      ]
    }
  ]
}
...
**Critical: NO LLM Usage**
- ✅ All processing is statistical (YAKE, Summa, scikit-learn)
- ✅ Enriches original metadata with cross-book similarities
- ❌ Does NOT add: best_practices, key_takeaways, common_pitfalls
- ❌ Those come from Tab 7 (LLM enhancement)
...
1. **Load Enriched Metadata**:
   ```python
   enriched = load_json("makinggames_metadata_enriched.json")
   ```

2. **Generate Markdown** (Jinja2 template):
   ```python
   from jinja2 import Template
   
   template = load_template("guideline.md.jinja2")
   markdown_output = template.render(enriched)
   save("makinggames_guideline.md", markdown_output)
   ```
   ...
   **Pages**: {{ chapter.start_page }}-{{ chapter.end_page }}
   ...
   ### Enriched Keywords
   {% for kw in chapter.keywords_enriched %}
   - {{ kw.term }} (score: {{ kw.score }})
   {% endfor %}
   
   ### Related Chapters
   {% for related in chapter.related_chapters %}
   - **{{ related.book }}**, Ch.{{ related.chapter }}: {{ related.title }} (relevance: {{ related.relevance_score }})
   {% endfor %}
   
   ### Key Concepts (Cross-Book Analysis)
   {% for concept in chapter.concepts_enriched %}
   - {{ concept.concept }} (relevance: {{ concept.relevance }}, appears in {{ concept.books }} books)
   {% endfor %}
   {% endfor %}
   ```

3. **Generate JSON** (structured format):
   ```python
   json_output = {
       "guideline": {
           "book": enriched["book"],
           ...
           "format_version": "1.0",
           "enrichment_method": enriched["enrichment_metadata"]["method"],
           "chapters": [
               {
                   "number": ch["number"],
                   "title": ch["title"],
                   "metadata": {
                       ...
                       "keywords_enriched": ch["keywords_enriched"],
                       "concepts": ch["concepts"],
                       "concepts_enriched": ch["concepts_enriched"],
                       "related_chapters": ch["related_chapters"]
                   },
                   "content": {
                       "summary": ch["summary"],
   ...
   save_json("makinggames_guideline.json", json_output)
   ```

**Key Points**:
- ✅ Pure template formatting (NO LLM calls)
- ✅ Both MD and JSON outputs for different use cases
- ✅ MD for human review, JSON for archival/tooling
- ✅ Can regenerate anytime from enriched metadata
- ✅ Preserves all statistical enrichments from Tab 4
...
**Input**: 
- `makinggames_taxonomy.json` (from Tab 3)
- `makinggames_metadata.json` (from Tab 2) - source book
- `makinggames_metadata_enriched.json` (from Tab 4) - source book (if exists)
- `makinggames_guideline.json` (from Tab 5) - source book
- All companion `*_guideline.json` files listed in taxonomy (from Tab 5)

**Output**: `makinggames_llm_package_{timestamp}.json` (720 KB for 12 books)
...
3. **Load Enriched Metadata for Each Book** (with graceful degradation):
   ```python
   books_data = []
   missing_books = []
   ...
   for book_info in book_list:
       ...
       # Try to load enriched metadata
       enriched_path = f"{book_name}_metadata_enriched.json"
       if file_exists(enriched_path):
           ...
           # Fallback: try basic metadata
           basic_path = f"{book_name}_metadata.json"
           if file_exists(basic_path):
               metadata = load_json(basic_path)
               books_data.append({
                   "name": book_name,
                   "metadata": metadata,
                   "note": "using_basic_metadata_fallback"
               })
           else:
               missing_books.append({
                   "name": book_name,
                   "reason": "metadata_not_found",
                   "expected_files": [enriched_path, basic_path]
               })
   ```
...
4. **Build Aggregate Package**:
   ...
   package = {
       "project": {
           "id": taxonomy.get("source_book", "unknown"),
           "generated": datetime.now().isoformat(),
           "source_taxonomy": f"{taxonomy['source_book']}_taxonomy.json",
           "package_version": "1.0"
       },
       "taxonomy": taxonomy,
       "books": books_data,
       ...
       "statistics": {
           ...
           "total_chapters": sum(len(b["metadata_enriched"]["chapters"]) 
                                 for b in books_data 
                                 if "metadata_enriched" in b),
           "missing_count": len(missing_books)
       }
   }
   
   output_path = f"workflows/llm_enhancement/tmp/{taxonomy['source_book']}_llm_package_{timestamp}.json"
...
```json
{
  ...
  "source_book": {
    "name": "makinggames",
    "metadata": { /* makinggames_metadata.json */ },
    "metadata_enriched": { /* makinggames_metadata_enriched.json if exists */ },
    "taxonomy": {
      "tiers": {
        "architecture": {
          "priority": 1,
          "books": ["architecturepatterns", "fluentpython"],
          "concepts": ["class", "inheritance"]
        }
      }
    },
    "guideline": { /* makinggames_guideline.json */ }
  },
  "companion_books": [
    {
      "name": "architecturepatterns",
      "guideline": { /* architecturepatterns_guideline.json */ }
    },
    {
      "name": "learningpython",
      "guideline": { /* learningpython_guideline.json */ }
    }
  ],
  "missing_books": [
    {
      ...
      "reason": "metadata_not_found",
      "expected_files": [
        "advanced_python_metadata_enriched.json",
        "advanced_python_metadata.json"
      ]
    }
  ],
  "statistics": {
...
**Key Points**:
- ✅ Pure file loading and combining (NO LLM)
- ✅ Graceful degradation for missing files
- ✅ Timestamped filename (multiple versions possible)
- ✅ Complete cross-book context for Tab 7
- ✅ Temporary file (can regenerate anytime)
...
**Input**: 
- `makinggames_llm_package_{timestamp}.json` (from Tab 6)
- `makinggames_guideline.md` or `.json` (from Tab 5)

**Output**: 
- `makinggames_guideline_enhanced.md` (primary, 400-1000 KB)
- `makinggames_guideline_enhanced.json` (optional, structured)
...
1. **Load Aggregate Package**:
   ```python
   # Get latest package
   package_path = get_latest_file("workflows/llm_enhancement/tmp/*_llm_package_*.json")
   aggregate = load_json(package_path)
   ```

2. **Load Guideline**:
   ```python
   guideline = load_json("makinggames_guideline.json")
   # or parse_markdown("makinggames_guideline.md")
   ```
...
3. **Build Cross-Book Context Index**:
   ...
   context_index = {}
   for book in aggregate["books"]:
       for chapter in book["metadata_enriched"]["chapters"]:
           key = f"{book['name']}_ch{chapter['number']}"
           context_index[key] = {
               "book": book["name"],
               "chapter": chapter["number"],
               "title": chapter["title"],
...
4. **LLM Enhancement Loop** (per chapter):
   ```python
   for chapter in guideline["chapters"]:
       # Get related content from aggregate
       related_content = []
       for rel in chapter["metadata"]["related_chapters"]:
           key = f"{rel['book']}_ch{rel['chapter']}"
           if key in context_index:
               related_content.append(context_index[key])
       
       # Build LLM prompt
       prompt = f"""
       You are enhancing a technical guideline chapter with scholarly depth.
       
       CURRENT CHAPTER:
       Title: {chapter["title"]}
       Keywords: {", ".join(chapter["metadata"]["keywords"])}
       Summary: {chapter["content"]["summary"]}
       
       CROSS-BOOK CONTEXT:
       {format_related_content(related_content)}
       ...
       5. **Cross-Reference Notes**:
          - Specific chapter references (e.g., "See Learning Python, Ch.2...")
          - Explain relevance of each reference
       
       OUTPUT FORMAT (Markdown):
       """
       
       # Call LLM
       enhanced = llm_provider.call(prompt, max_tokens=2000)
       
       # Parse and integrate enhancements
       chapter_enhanced = parse_llm_response(enhanced)
       chapter.update(chapter_enhanced)
   ```

5. **Generate Enhanced Markdown**:
   ```python
   template = load_template("guideline_enhanced.md.jinja2")
   markdown_output = template.render(guideline)
   save("makinggames_guideline_enhanced.md", markdown_output)
   ```

**Output Example**:
```markdown
# Making Games with Python and Pygame (LLM-Enhanced)

## Chapter 1: Installing Python and Pygame

### Enhanced Summary
This chapter provides a foundational introduction to setting up Python and
Pygame, essential tools for game development. The installation process follows
industry-standard practices for Python package management, as emphasized across
multiple authoritative sources including *Learning Python* (Ch.2)[^1] and 
*Fluent Python* (Ch.1)[^2]. Understanding proper environment setup is critical
for maintainable projects, a theme echoed in *Architecture Patterns* which
stresses the importance of reproducible development environments[^3].

### Key Takeaways
- Use virtual environments to isolate project dependencies and avoid conflicts
- Verify Pygame installation with `import pygame` before starting development
- Follow PEP 668 standards for modern Python package management
- Document all dependencies in `requirements.txt` for team collaboration

### Best Practices
- **Use pip for package management**: Following PEP 453 recommendations, pip
  is the standard tool for installing Python packages[^4]
- **Create virtual environments**: Isolates dependencies per project, preventing
  version conflicts (see *Python Distilled*, Ch.9)[^5]
- **Test installation before coding**: Ensures smooth workflow and catches
  configuration issues early[^1]

### Common Pitfalls
- **PATH configuration errors**: Python not found in system PATH
  - **Solution**: Use `py` launcher on Windows, verify with `which python` on Unix
- **Version incompatibility**: Pygame requires Python 3.7+
  - **Solution**: Check version with `python --version` before installation
- **Missing dependencies**: Some Pygame features require additional libraries
  - **Solution**: Install system dependencies (e.g., SDL2) before Pygame

### Cross-References
- **Learning Python, Ch.2**: Comprehensive Python installation guide covering
  multiple platforms and troubleshooting
- **Architecture Patterns, Ch.1**: Project structure best practices, including
  environment management and dependency handling
- **Fluent Python, Ch.1**: Understanding Python's data model aids in effective
  use of Pygame's object-oriented APIs

### Citations
[^1]: Lutz, Mark. *Learning Python* (6th ed.). Sebastopol, CA: O'Reilly Media, 2020.
[^2]: Ramalho, Luciano. *Fluent Python* (2nd ed.). Sebastopol, CA: O'Reilly Media, 2022.
[^3]: Percival, Harry and Gregory, Bob. *Architecture Patterns with Python*. Sebastopol, CA: O'Reilly Media, 2020, 48-50.
[^4]: van Rossum, Guido. "PEP 453 - Explicit Bootstrapping of pip in Python Installations." Python.org, 2013.
[^5]: Beazley, David. *Python Distilled*. Boston: Addison-Wesley, 2021, Chapter 9.
```

**What LLM Adds**:
- ✅ Enhanced summaries with cross-book synthesis
- ✅ Key takeaways (actionable insights)
- ✅ Best practices with scholarly citations
- ✅ Common pitfalls with practical solutions
- ✅ Cross-reference notes with relevance explanations
- ✅ Chicago-style citations
- ✅ Multi-book perspective and integration

**What LLM Does NOT Do**:
- ❌ Modify canonical metadata (stays in `*_metadata_enriched.json`)
- ❌ Change statistical enrichments (TF-IDF scores, similarity rankings)
- ❌ Alter chapter structure or organization
- ❌ Rewrite original summaries (enhances, doesn't replace)

**Critical Points**:
- ✅ THE ONLY tab that uses LLM
- ✅ All prior tabs (1-6) are statistical/template-based
- ✅ Uses aggregate package for full cross-book context
- ✅ Pure narrative enhancement layer
- ✅ Can regenerate if LLM model improves
- ✅ Preserves all statistical analysis from Tab 4

---

### Key Architectural Principles

**1. Separation of Statistical vs LLM Work**:
- **Tabs 1-6**: NO LLM (YAKE, Summa, scikit-learn, templates)
- **Tab 7 ONLY**: LLM enhancement (narrative layer)

**2. Single Source of Truth**:
- `*_metadata_enriched.json` (Tab 4) = Canonical data
- Guidelines (Tab 5, Tab 7) = Formatted views of canonical data

**3. Graceful Degradation**:
- Tab 6: Falls back to basic metadata if enriched not found
- Tab 7: Can skip books with missing data

**4. Regenerable Outputs**:
- Tab 5: Regenerate guidelines from enriched metadata
- Tab 6: Regenerate aggregate package anytime
- Tab 7: Regenerate LLM enhancements with better models

**5. Temporary vs Permanent Files**:
- **Permanent**: `*_metadata_enriched.json`, `*_guideline.md/json`
- **Temporary**: `*_llm_package_{timestamp}.json` (can delete after Tab 7)
   ```python
   # Just load enriched metadata - no merging!
   for book in taxonomy_books:
       enriched = load(f"{book}_metadata_enriched.json")
       package["books"].append(enriched)
   ```

### What Gets Removed

❌ **Tab 4 (Cache Merge)** - Deprecated monolithic approach
❌ **Tab 6 (Base Guideline)** - Functionality split:
  - Statistical extraction → Stays in Tab 2
  - LLM synthesis → Moves to Tab 4 (Enrichment)
  - Formatting → Moves to Tab 5 (new simplified version)
❌ **Tab 7 (LLM Enhancement)** - Merged into Tab 4

### Current vs Proposed Comparison

| Aspect | Current (7 Tabs) | Proposed (5 Tabs) |
|--------|-----------------|-------------------|
| **Workflow Order** | Enrichment → Cache → Taxonomy | Taxonomy → Enrichment |
| **LLM Usage** | Tab 6 (Base) + Tab 7 (Enhancement) | Tab 4 only (Comprehensive Enrichment) |
| **Output Files** | Separate enriched + guideline | Merged metadata_enriched.json |
| **Guideline Gen** | LLM-powered synthesis | Simple formatting |
| **Aggregation** | Would need merging logic | Trivial file loading |
| **Source of Truth** | Scattered across files | Single enriched metadata |

---

## Current Workflow State

### Current Tab Order (PROBLEMATIC)

```python
WORKFLOWS = {
    "tab1": {"name": "PDF to JSON"},
    "tab2": {"name": "Metadata Extraction"},
    "tab3": {"name": "Metadata Enrichment"},      # ⚠️ Comes BEFORE taxonomy
    "tab4": {"name": "Cache Merge"},              # ⚠️ Deprecated monolithic
    "tab5": {"name": "Taxonomy Setup"},           # ⚠️ Comes AFTER enrichment  
    "tab6": {"name": "Base Guideline"},           # ⚠️ Batch only, has LLM
    "tab7": {"name": "LLM Enhancement"}           # ⚠️ Redundant with Tab 6
}
```

### Current Scripts (Need Changes)

**Tab 2**: `workflows/metadata_extraction/scripts/generate_metadata_universal.py`
- Status: ✅ Uses StatisticalExtractor (YAKE + Summa)
- Minor: Still has some hardcoded patterns to remove

**Tab 3**: `workflows/metadata_enrichment/scripts/generate_chapter_metadata.py`
- Status: ❌ Monolithic (14 hardcoded books)
- Issue: Cannot process single files from UI
- Required: Replace with new per-book enrichment script

**Tab 5**: `workflows/taxonomy_setup/scripts/generate_concept_taxonomy.py`
- Status: ✅ FIXED - Auto-generates filename from source
- Recent commit: "fix(taxonomy): Auto-generate taxonomy filename from source book"

**Tab 6**: `workflows/base_guideline_generation/scripts/chapter_generator_all_text.py`
- Status: ⚠️ CRITICAL ISSUE - Has `USE_LLM_SEMANTIC_ANALYSIS = True`
- Required: **Delete or repurpose** - functionality moves to Tab 4 + Tab 5

**Tab 7**: `workflows/llm_enhancement/scripts/integrate_llm_enhancements.py`
- Status: ⚠️ Will be merged into new Tab 4 comprehensive enrichment

---

## Critical Issues Identified

### Issue 1: LLM in Base Guideline Generation (CRITICAL)
...
**Problem**:
- Base guideline calls LLM for:
  - Semantic concept extraction
  - Cross-reference validation
  - Annotation generation
- Defeats purpose of separate LLM Enhancement workflow
- Adds unnecessary cost before main LLM processing

**Required Fix**:
```python
# LLM semantic analysis DISABLED for base guideline generation
# Base guidelines use keyword/statistical methods only
# LLM enhancement happens in separate workflow (Tab 7: LLM Enhancement)
USE_LLM_SEMANTIC_ANALYSIS = False  # DISABLED - Statistical methods only
```

**Additional Changes Required**:
1. Line 664: `_try_llm_annotation()` - Skip LLM calls
2. Line 1450+: Keyword-based matching only
3. Line 1500+: Skip LLM cross-reference validation
4. Ensure fallback logic works correctly

**Impact**: HIGH - Changes workflow semantics

**Status**: ⏸️ PENDING - Must fix before implementation

### Issue 2: Workflow Order (HIGH PRIORITY)

**Current Problem**: Enrichment runs before Taxonomy
- Enrichment needs to know which companion books to compare against
- But taxonomy (which defines book list) runs AFTER enrichment

**Required Changes**:

1. **UI Tab Order** (`ui/main.py`):
```python
# Rename tab3 → tab4, tab4 → tab5, tab5 → tab3
WORKFLOWS = {
    "tab1": {"name": "PDF to JSON", ...},
    "tab2": {"name": "Metadata Extraction", ...},
    "tab3": {"name": "Taxonomy Setup", ...},      # MOVED UP
    "tab4": {"name": "Metadata Enrichment", ...}, # MOVED DOWN
    "tab5": {"name": "Cache Merge", ...},         # MOVED DOWN
    "tab6": {"name": "Base Guideline", ...},
    "tab7": {"name": "LLM Enhancement", ...}
}
```

2. **UI Template** (`ui/templates/index.html`):
- Update tab IDs and labels
- Update tab navigation

3. **Enrichment Script** (`generate_chapter_metadata.py`):
- Add `--taxonomy` argument
- Read book list from taxonomy
- Use book list to filter enrichment targets

**Impact**: MEDIUM - Improves logical flow

**Status**: ⏸️ APPROVED - Implementation pending

### Issue 3: Monolithic Scripts (MEDIUM PRIORITY)

**Problem**: Cannot process individual books from UI

**Affected Scripts**:
1. `chapter_generator_all_text.py` - Hardcoded "Learning Python Ed6"
2. `generate_chapter_metadata.py` - Processes 14 books at once

**Required Changes**:

**Create**: `enrich_metadata_per_book.py`
```bash
python3 enrich_metadata_per_book.py \
  --input makinggames_metadata.json \
  --taxonomy makinggames_taxonomy.json \
  --output makinggames_metadata_enriched.json
```

**Create**: `generate_guideline_per_book.py`
```bash
python3 generate_guideline_per_book.py \
  --input makinggames_metadata_enriched.json \
  --taxonomy makinggames_taxonomy.json \
  --output-md makinggames_guideline.md \
  --output-json makinggames_guideline.json
```

**Status**: ⏸️ IDENTIFIED - Implementation pending

### Issue 4: Missing Guideline JSON Output (LOW PRIORITY)

**Problem**: Base guideline only generates MD, not JSON

**Required**: 
- Generate both `makinggames_guideline.md` and `makinggames_guideline.json`
- JSON needed for LLM aggregation in Tab 7
...
### 1. File Naming Convention (COMMITTED)
...
### 2. Workflow Order (APPROVED)

**Change**: Swap Taxonomy and Enrichment

**New Order**:
1. PDF → JSON
2. Metadata Extraction
3. **Taxonomy Setup** ← MOVED UP
4. **Metadata Enrichment** ← MOVED DOWN
5. Cache Merge
6. Base Guideline
7. LLM Enhancement
...
### 3. LLM Usage (FINALIZED)

**LLM ONLY in Tab 7** (LLM Enhancement)

**NO LLM in**:
- Tab 2: Metadata Extraction (YAKE + Summa/TextRank)
- Tab 3: Taxonomy Setup (JSON generation)
- Tab 4: Metadata Enrichment (YAKE + Summa + scikit-learn TF-IDF/cosine similarity)
- Tab 5: Cache Merge (data processing)
- Tab 6: Base Guideline (keyword/template-based)

**Statistical Libraries Used**:
1. **YAKE** (Yet Another Keyword Extractor) - Unsupervised keyword extraction
2. **Summa** (TextRank) - Extractive summarization and concept identification
3. **scikit-learn** - TF-IDF vectorization, cosine similarity for cross-book analysis

**Status**: ⏸️ FINALIZED - Must fix Tab 6

### 4. Guideline Outputs (DECIDED)

**Generate Both**:
- `{source}_guideline.md` - Human-readable
- `{source}_guideline.json` - Machine-readable for LLM

**Status**: ⏸️ DECIDED - Implementation pending

### 5. Aggregation Strategy (DECIDED)

**Approach**: Multiple Small Calls with Compact Metadata

**CRITICAL TIMING CLARIFICATION** (Discussed multiple times):
- Packages created DURING Tab 7 (LLM Enhancement), NOT during enrichment
- Cached in `workflows/llm_enhancement/cache/aggregated_packages/`
- Reused within same session (7-day TTL)
- Sources: Guideline JSON (priority) OR enriched metadata JSON (fallback)

**Package Size**: 720 KB for 12 books (60 KB per book)

**Cost**: $7.02 per book (28% cheaper than alternatives)

**Status**: ⏸️ DECIDED - Implementation pending

### 5a. Metadata Enrichment Purpose (CLARIFIED)

**What "Enrichment" Means** (Discussed extensively):

**NOT LLM-based** - Uses all three statistical libraries:
1. **YAKE** - Keyword extraction and scoring
2. **Summa (TextRank)** - Concept identification and extractive summarization
3. **scikit-learn** - TF-IDF vectorization and cosine similarity for cross-book analysis

**Statistical Methods**:
- TF-IDF for cross-book similarity
- Cosine similarity for concept matching
- YAKE for domain-agnostic keyword extraction
- TextRank for multi-document summarization
- Statistical ranking and filtering

**Merges Multiple Sources**:
- Basic metadata (`{book}_metadata.json`) - Contains YAKE keywords + Summa summaries
- Taxonomy (`{book}_taxonomy.json`) - Companion book list
- Source JSON (`{book}.json`) - Full text for TF-IDF analysis

**Enhances With**:
- Cross-book similarity scores (scikit-learn cosine similarity)
- Concept relevance rankings (YAKE + TF-IDF)
- Statistical cross-references (TextRank + cosine similarity)
- Filtered/prioritized keywords (YAKE scores)

**Output**: `{book}_metadata_enriched.json` (~50 KB)
- Used as fallback source for aggregation (if guideline JSON missing)
- Enables statistical pre-filtering (Part 2 of DOMAIN_AGNOSTIC plan)
- NO LLM-generated content

**Status**: ⏸️ CLARIFIED - Replaces monolithic script

### 6. Domain-Agnostic Metadata (IN PROGRESS)

**Approach**: Replace hardcoded Python keywords with statistical NLP

**Three Libraries** (All actively used):
1. **YAKE (Yet Another Keyword Extractor)**
   - Repository: https://github.com/LIAAD/yake
   - Usage: Unsupervised keyword extraction (Tab 2, Tab 4)
   - Status: ✅ INTEGRATED in `statistical_extractor.py`

2. **Summa (TextRank for summarization and keywords)**
   - Repository: https://github.com/summanlp/textrank
   - Usage: Extractive summarization and concept identification (Tab 2, Tab 4)
   - Status: ✅ INTEGRATED in `statistical_extractor.py`

3. **scikit-learn (TF-IDF, cosine similarity)**
   - Repository: https://github.com/scikit-learn/scikit-learn
   - Usage: Cross-book similarity analysis (Tab 4)
   - Status: ⏸️ PLANNED for enrichment script

**Current State**:
- Part 1: ~70% complete (YAKE + Summa integrated)
- Part 2: Not started (TF-IDF filtering for LLM cost reduction)

**Status**: ⏸️ IN PROGRESS - Documented in DOMAIN_AGNOSTIC_IMPLEMENTATION_PLAN.md

**Status**: Part 1 of existing plan ~70% complete

**Remaining**:
- Complete StatisticalExtractor integration
- Remove final hardcoded patterns
- Test on multiple domains

**Status**: ⏸️ IN PROGRESS - Continue existing work
...
1. **Aggregate packages**: Created DURING Tab 7, cached for reuse, NOT stored during enrichment
2. **Metadata enrichment**: Merges basic metadata + taxonomy + source, enhances with TF-IDF/similarity
3. **Package sources**: Guideline JSON (priority) OR enriched metadata JSON (fallback)
4. **Cache location**: `workflows/llm_enhancement/cache/aggregated_packages/`
5. **Cache TTL**: 7 days (configurable)
6. **NO LLM in enrichment**: Statistical methods only (we discussed this extensively)

These decisions were made multiple times throughout our conversations. Implementation must reflect these choices exactly.
...
#### Task 1.1: Disable LLM in Base Guideline Generation

**File**: `workflows/base_guideline_generation/scripts/chapter_generator_all_text.py`

**Changes**:
1. Line 108: Set `USE_LLM_SEMANTIC_ANALYSIS = False`
2. Add comment explaining why disabled
3. Verify fallback logic works
4. Test script runs without LLM calls

**Validation**:
```bash
# Should NOT make any LLM API calls
python3 workflows/base_guideline_generation/scripts/chapter_generator_all_text.py \
  --input workflows/pdf_to_json/output/textbooks_json/makinggames.json
```

**Success Criteria**:
- ✅ No LLM API calls made
- ✅ Uses keyword/statistical methods only
- ✅ Generates guideline.md successfully
...
**New Tests**:
1. `test_no_llm_in_base_guideline.py` - Verify no LLM calls
2. `test_statistical_guideline_generation.py` - Verify output quality

**Status**: ⏸️ PENDING

---

### Phase 2: Workflow Reordering (Week 1-2)

**Priority**: HIGH - Improves logical flow

#### Task 2.1: Update UI Tab Configuration
...
WORKFLOWS = {
    ...
    "tab4": {
        ...
        "requires_taxonomy": True  # NEW
    },
    "tab5": {"name": "Cache Merge", ...},
    "tab6": {"name": "Base Guideline", ...},
    "tab7": {"name": "LLM Enhancement", ...}
}
```
